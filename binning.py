...
def binning_widget(df: pd.DataFrame, group_col: str):
    ...
    mode = st.sidebar.radio(
        "Binning mode", ["Equal width", "Manual"], horizontal=True, key=f"bin_mode_{k}"
    )
    ...
    if mode == "Equal width":
        n_bins = st.sidebar.slider("Number of bins", 2, 15, 4, key=f"bin_nbins_{k}")
        cut, edges = pd.cut(series, bins=n_bins, include_lowest=True, retbins=True)
        labels = [f"{round(l, 2)}–{round(r, 2)}" for l, r in zip(edges[:-1], edges[1:])]
