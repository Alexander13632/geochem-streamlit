...
for idx, g in enumerate(sorted(df[group_col].dropna().unique())):
    ...
    with st.sidebar.expander(str(g), expanded=False):
        ...
        size = st.slider("Size (px)", 2, 40, 10, key=f"sz_{g}")
        alpha = st.slider("Opacity (%)", 10, 100, 90, key=f"op_{g}") / 100.0
        styles[g] = {"color": color, "symbol": symbol, "size": size, "opacity": alpha}
        if show:
...
for trace in fig.data:
    if trace.name in styles:
        trace.update(marker=dict(opacity=styles[trace.name]["opacity"]))
